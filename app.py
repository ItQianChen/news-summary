...
with st.sidebar:
    ...
    with st.expander("🤖 AI 配置", expanded=True):
        ...
        embedding_model = st.text_input(
            "EMBEDDING_MODEL",
            value=env_vars.get("EMBEDDING_MODEL", ""),
            help="可选。用于热点标题向量化聚类；不填写时，事件聚类退回规则模式。",
        )
    ...
    with st.expander("📊 抓取数量限制", expanded=True):
        ...
        focus_limit = st.number_input("最终报告关注事件数", min_value=1, value=report_cfg.get("focus_limit", 10))

    with st.expander("📝 报告生成配置", expanded=True):
        render_mode_options = {
            "rule": "规则版",
            "ai": "AI版",
            "all": "全部生成",
        }
        current_render_mode = str(report_cfg.get("render_mode", "rule")).strip().lower()
        if current_render_mode not in render_mode_options:
            current_render_mode = "rule"
        render_mode = st.selectbox(
            "日报生成模式",
            options=list(render_mode_options.keys()),
            index=list(render_mode_options.keys()).index(current_render_mode),
            format_func=lambda key: render_mode_options[key],
            help="规则版使用本地代码拼装；AI版使用日报总编辑 Prompt；全部生成会同时输出两份 Markdown。",
        )
...
if run_btn:
    ...
    settings["report"]["render_mode"] = render_mode
...
if st.session_state.get("run_pipeline", False):
    with st.spinner("流水线运行中，正在抓取数据并调用大模型进行分析，请耐心等待..."):
        try:
            ...
            report_paths = run_pipeline("manual")
            st.session_state["report_paths"] = [str(path) for path in report_paths]
            st.session_state["report_path"] = str(report_paths[0]) if report_paths else ""
...
if not md_files:
    ...
    default_index = 0
    preferred_paths = st.session_state.get("report_paths") or []
    if not preferred_paths and st.session_state.get("report_path"):
        preferred_paths = [st.session_state["report_path"]]

    if preferred_paths:
        resolved_targets = {str(Path(path).resolve()) for path in preferred_paths}
        for i, f in enumerate(md_files):
            if str(f.resolve()) in resolved_targets:
                default_index = i
                break
        ...
        if has_toc:
            ...
            for line in content.split("\n"):
                ...
                if match:
                    ...
                    html_toc_items.append(
                        f"<li style='margin-left: {(level-1)*15}px; margin-bottom: 8px;'><a href='#{anchor_id}' target='_self' style='text-decoration:none; color:inherit; font-size:14px; display:block;'>{title}</a></li>"
                    )
